...
foreach ($array['results'] as $v) {
	...
	if (($defaultloc == "assign") && (!empty($defaultlat))) {
		echo "<item>\n";
		$url = "https://twitter.com/" . $v['from_user'] . "/status/" . $v['id_str'];
			...
			echo $geolat;
			echo $geolong;
			if ($v['entities']['media'][0]['media_url']) {echo "<media:content url=\"" . $v['entities']['media'][0]['media_url'] . "\" type=\"image/jpeg\"></media:content>\n";}
			echo "<link>$url</link>\n";
			echo "<guid>$url</guid>\n";
		echo "</item>\n";
	} elseif ($v['geo']['coordinates'][0]) {
		echo "<item>\n";
		$url = "https://twitter.com/" . $v['from_user'] . "/status/" . $v['id_str'];
			echo "<title>@" . $v['from_user'] . " tweets: " . $v['text'];
			if ($linkinfield == "title") {echo " " . $url;}
